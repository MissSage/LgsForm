 ...
 * @returns
 */
export const excuteIdentify = (url: string, params: IdentifyParameters) => {
...
export const gotoAndHighLight = async (
  view?: MapView,
  target?: Graphic,
  options?: { zoom?: number; duration?: number; avoidHighlight?: boolean }
) => {
  ...
  if (extent) {
    ...
    await view?.goTo(
      { zoom: options?.zoom || 24, target: target?.geometry },
...
export const submitBurstAnalysGPJob = (layerdbname, oid) => {
  return geoprocessor.submitJob(
    window.SITE_CONFIG.gisService
      + 'BurstPipeAnalyzenanbu/GPServer/BurstPipeAnalyzenanbu',
    {
...
export const submitConnectAnalysGPJob = (
  ...
  return geoprocessor.submitJob(
    window.SITE_CONFIG.gisService
      + 'ConnectedAnalyzenanbu/GPServer/ConnectedAnalyzenanbu',
    {
      pipefeatureclassname: layerdbname,
      pipeobjectid: oid,
      applystop,
...
export const excutePrintTask = (view?: MapView, template?: PrintTemplate) => {
  ...
  const url = window.SITE_CONFIG.gisUtilitiesService
    + window.SITE_CONFIG.gisPrintingToolsGPService
...
export const getLayerOids = async (
  ...
  const getTab = async (index: number) => {
    ...
    try {
      ...
      const oidUrl = window.SITE_CONFIG.gisService + window.SITE_CONFIG.gisPipeDataService

<template>
  <el-form ref="refForm" :inline="true" class="inline-form" :model="state.dataForm" :size="config.size || 'default'"
    :label-position="config.labelPosition || 'left'" :label-width="config.labelWidth || '100px'"
    :validate-on-rule-change="false" @submit.prevent>
    <template v-for="(group, i) in config.group" :key="i">
      <div v-if="!group.hidden" :style="group.styles" class="inline-group">
        <FieldSet v-if="group.fieldset" :type="group.fieldset?.type"
          :style="'width:' + (group.fieldset.width || '100%')">
          <span class="title">{{ group.fieldset?.desc }}</span>
          <template v-if="group.fieldset.right?.length">
            <template v-for="(rightitem, j) in group.fieldset.right" :key="j">
              <div :class="rightitem.className">
                <template v-for="(item, k) in rightitem.items" :key="k">
                  <template v-if="!item.hidden">
                    <FormItem v-if="item.field" v-model="state.queryParams[item.field]" :config="item"
                      @change="(val: any) => group.fieldset?.handleQuery && group.fieldset.handleQuery(val, state.queryParams, item, group)" />
                    <FormItem v-else :config="item"
                      @change="(val: any) => group.fieldset?.handleQuery && group.fieldset.handleQuery(val, state.queryParams, item, group)" />
                  </template>
                </template>
              </div>
        ...
        <template v-for="(item, j) in group.fields" :key="j">
          <template v-if="!item.hidden">
            <el-card v-if="item.type === 'card-table'" :style="item.itemContainerStyle || 'width:100%;'">
              <template v-if="item.title || item.titleRight?.length" #header>
                <div v-if="item.titleRight?.length" class="right-wrapper">
                  <template v-for="(rightitem, k) in item.titleRight" :key="k">
                    <div :class="rightitem.className">
                      <template v-for="(obj, z) in rightitem.items" :key="z">
                        <template v-if="!obj.hidden">
                          <FormItem v-if="obj.field" v-model="state.queryParams[obj.field]" :config="obj"
              ...
              <FormTable v-if="item.field" v-model="state.dataForm[item.field]" :config="item.config"
                @change="(val: any) => handleFormItemChange(val, item, group)" />
            </el-card>
            <FormTable v-else-if="item.type === 'table' && item.field" v-model="state.dataForm[item.field]"
              :style="item.itemContainerStyle || 'width:100%;'" :config="item.config"
              @change="(val: any) => handleFormItemChange(val, item, group)" />
            <el-form-item v-else :style="item.itemContainerStyle" :class="item.className" :label="item.label"
              :prop="item.field" :label-width="item.labelWidth || config.labelWidth"
              :rules="item.type !== 'text' ? item.rules : []">
              <FormItem v-model="state.dataForm[item.field!]" :config="item" :row="state.dataForm" :size="config.size"
                @change="(val: any) => handleFormItemChange(val, item, group)" />
            </el-form-item>
          </template>
        </template>
        <template v-if="group.groupBtns?.btns.length">
          <div :style="group.groupBtns.styles" :class="group.groupBtns.className">
            <Button v-for="(btn, j) in group.groupBtns?.btns" :key="j" :config="btn" />
          </div>
        </template>
      </div>
    </template>

<template>
  <el-button v-if="typeof config?.perm === 'function' ? config.perm(row) : config?.perm" v-show="
    !(typeof config.hide === 'function' ? config.hide(row) : config.hide)
  " :loading="
  typeof config.loading === 'function'
    ? config.loading(row)
    : config.loading
" :size="config.size || size || 'default'" :text="config.isTextBtn" :link="config?.link || false"
    :icon="config.svgIcon" :style="
...
import { IButton, ISize } from '../../lgsform';

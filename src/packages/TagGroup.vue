<template>
  ...
  >
    ...
    >
      ...
      <span
        class="tag-text"
        :style="item.value === modelValue ? 'color:' + config.activeTextColor : ''"
      >
        {{ item.label }}
      </span>
      <span
...
watch(
  () => store.app.isDark,
  (newVal: boolean) => {
    state.defaultBgColor = newVal ? '#222536' : '#fff'
    state.defaultBorderColor = newVal ? '#373858' : '#dcdfe6'
  }
)
const state = reactive<{
  ...
  defaultBgColor: store.app.isDark ? '#222536' : '#fff',
  defaultBorderColor: store.app.isDark ? '#373858' : '#dcdfe6'
})
